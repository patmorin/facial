...
%  \end{equation}
 ...
 \begin{proof}
 Let $F_1^H,\ldots,F_{f_H}^H$ be the inner faces of $H$ in the order considered by the procedure. and let $G_i^H$ be the graph induced by  
 \begin{equation}
 ...
 \begin{enumerate}
  \item $|B_{k-1} \cap V(F_i^H)|=1$. In this case, one of the anchors of $F_i^H$ is in $B_{k-1}$. Thus, since $F_i^H$ is a cycle, $F_i^H \setminus B$ is a path. This path is connected by the other anchor of $F_i^H$ to $G_{i-1}^H$, which is connected. Thus, $G_i^H$ is also connected.
  
  \item $B_{k-1} \cap V(F_i^H) = \emptyset$. In this case, none of the anchors $a_1,a_2$ of $F_i^H$ are in $B_{k-1}$ and since $a_1,a_2 \in G_{i-1}^H$, $a_1,a_2 \notin B_k$. $F_i^H$ is a cycle, so by Claim \ref{claim:blocking_out_1}, $F_i^H \setminus B$ is a path which contains both $a_1$ and $a_2$. This path is connected to $G_{i-1}^H$, which is connected by the induction hypothesis, therefore $G_i^H$ is also connected.
 \end{enumerate}
 This completes the proof.
 \end{proof}
