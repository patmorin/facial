...
\begin{proof}
By \lemref{non-simple}, we may assume that $G$ is simple.
...
\lemref{blocking-set}, $G$ has a blocking set $B$ such that $\block_B(G)$
...
12-colouring of outerplanar graphs whereas our \thm{main} provides an
...
\begin{proof}
   ...
   Note that $W$ contains only vertices from $V_i(G)$ and $V_{i+1}(G)$
   for some $i$.  Remove from $W$ all vertices in $V_{i+1}(G)$ to
   obtain a cyclic sequence $W'$ of vertices from $V_i(G)$.  For any
   two consecutive vertices $u\neq w$ in $W'$, we add the edge $uw$
   to $G^+$ inside the face $F$.  Note that the resulting graph $G^+$
   is still plane and that, for all $i$, $V_i(G)=V_i(G^+)$.
   From this point onward, we use the notation $V_i=V_(G)=V_i(G^+)$.

   For each $i$, $G^+[V_i]$ is outerplane.  To colour $G$ we use
   \thmref{main} to facially nonreptitively colour $G^+[V_i]$ with
   ...
   have the same outside face so, in this case, $P$ is not a repetition
   since our colouring is facially nonrepetitive for $G^+[V_0]$.

   Therefore, $F$ is an inner face and 
   all vertices on $F$ are in $V_i\cup V_{i+1}$ for some $i$.  There are three
   cases to consider:
   \begin{enumerate}
      \item If $V(P)\subseteq V_i$, then we claim that $P$ is a facial path
        in $G^+[V_i]$.
      \item If $V(P)\subseteq V_{i+1}$, then we claim that $P$ is a facial
        path on the outer face of $G^+[V_{i+1}]$. (Easier)
      \item Otherwise, $V(P)$ contains vertices from both $V_i$ and
        $V_{i+1}$.
        In this case, we claim that the colour sequence determined by
        $P$ is of the form $A_0,B_1,A_1,\ldots,B_\ell,A_\ell$, where
        each $A_j$ is a nonrepetitive sequence obtained from vertices
        in $V_{i+1}$ and $B_1,\ldots,B_\ell$ is a nonrepetitive sequence
        obtained from vertices in $V_i$.  In particular, if we consider
        the sequence $P'$ obtained by removing vertices in $V_{i+1}$
        from $P$, then $P'$ is a facial path in $G^+[V_i]$.
   \end{enumerate}
\end{proof}
