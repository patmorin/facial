...
$B$. For each consecutive pair of vertices $uw$ in $W'$ we add the edge
 ...
 the blocking graph $\block_B(G)$. See \figref{blocking-graph}
...
\begin{lem}[K{\"u}ndgen and Pelsmajer \cite{kundgen2008nonrepetitive}] \lemlabel{level_pattern_palindrome_free}
 ...
 as $c(v)=s_{\lambda(v)}$. %Thus each $v \in V(G)$ gets the colour
...
\begin{rem}\remlabel{bw}
   We remark that the proof of \lemref{even-biconnected} actually proves
   something stronger than just requiring the inclusion or exclusion of
   $v\in B$.  We can specify an edge $bw$ on the outer face of $G$ and
   obtain a blocking set $B$ such that $\block_B(G)$ is an even cycle,
   $b\in B$ and $w\not\in B$.  This is used in \appref{biconnected}.
\end{rem}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \begin{figure}
       \begin{center}
          \includegraphics{figs/block-merge}
       \end{center}
