...
that, for every path $P$ in $G$, the sequence of colours of vertices in
$P$ is not a repetition.  The \emph{nonrepetitive chromatic number}, or
\emph{Thue chromatic number}, of $G$, denoted $\pi(G)$, is the minimum
number of colours required to nonrepetitively colour $G$.  In this
setting, Thue's result states that, for all $n\ge 1$, the path $P_n$
on vertices has $\pi(P_n)\le 3$.  Since its introduction, nonrepetitive
...
is a contiguous subsequence of a facial walk; see \secref{preliminaries}
...
The upper bound of $4^t$ for $t$-trees is tight if $t=1$ (trees), but it
is not known if it is tight for other values of $t$. Even the upper
bound of 12 for outerplanar graphs might not be tight, as no outerplanar
...
%nonrepetitive.
%A \emph{plane graph} $G$ is a fixed embedding of a graph in the plane such that its edges intersect only at their endpoints. An \emph{outerplane} graph $G$ is a plane graph such that all the vertices of $G$ are adjacent to the outer face of $G$. 
...
component} of a graph $G$ is a maximal subset of vertices of $G$
that induces a $k$-connected subgraph.  We use \emph{biconnected} as
a synonym for $2$-connected. A \emph{bridge} in a graph $G$ is an edge
whose removal increases the number of connected components.  A graph is
\emph{bridgeless} if it has no bridges.
...
An \emph{ear} in a simple outerplane graph is an inner face that is
...
\begin{lem}[Havet \etal~\cite{havet2011facial}]\lemlabel{interleave}
...
\begin{thm}[Alon \etal~\cite{alon2002nonrepetitive}]\thmlabel{tree}
...
\begin{lem}\lemlabel{non-simple}
  ...
  graph obtained by adding parallel edges and/or loops to $G$.  Then, any
...
we use \thmref{cycle} to nonrepetitively 4-colour the cycle, $C$, that
...
with nonrepetitive sequences taken from $T$; by \lemref{interleave},
...
Finally, we remind the reader that, although \lemref{biconnected} and
\corref{biconnected-out} provide blocking sets that include only one
...
defines the embedding of $\block_B(G)$. See \figref{blocking-graph}
...
\item The colouring of $P$ is of the form $A_0,B_1,A_1$, where
  ...
  (by \obsref{shit}).  \lemref{interleave} therefore implies that the
...
\item The colouring of $P$ is of the form $B_0,A_1,B_1$, where $A_1$
  ...
  (possibly empty) outer-facial paths in $\block_B(G)$ (by \obsref{shit}).
...
a (outer) facial path in $\block_B(G)$ (by \obsref{shit0}).  Again,
\lemref{interleave} implies that the resulting colour sequence is
nonrepetitive.
\end{proof}
...
in \secref{even-blocking-graph} by proving the existence of a blocking
set $B$ such that $\block_B(G)$ has no odd cycles.

For any graph, $G$, a \emph{levelling} of $G$ is a function
$\lambda\colon V(G)\to \{0, 1, 2,\dots\}$ such that for
each $uv\in E(G)$, $|\lambda(u)-\lambda(v)|\leq 1$. The
level pattern of a path $v_1,\ldots,v_k$ is the sequence
$\lambda(v_1),\lambda(v_2),\ldots,\lambda(v_k)$.  

A \emph{palindrome} is a sequence, $s_1,\ldots,s_k$, that equal to its
reversal, i.e., $s_i=s_{k-i+1}$ for all $i\in \{1,\ldots,k\}$.  A sequence
is \emph{palindrome-free} if it none of its substrings is a palindrome.
...
\begin{lem}[K{\"u}ndgen and Pelsmajer \cite{kundgen2008nonrepetitive}] \lemlabel{level_pattern_palindrome_free}
 ...
 $|P_1|=|P_2|$ in $G$ is repetitively coloured by $c$, then $P_1$
...
\begin{proof}
 ...
 If there exists a vertex $v$ of $G$ of degree 1, then let
...
that $P$ is on the outer face of $G$. We will use the following claim:
 ...
 is of the form $A_0,B_1,A_1,\ldots,B_\ell,A_\ell$ where $B_1,\ldots,B_\ell$
...
The proof of the following lemma requires more work than any other result
in this paper and is excluded due to space limitations.  It can, however, 
be found in the full version \cite{bose.dujmovic.ea:new}.
...
%  \corref{biconnected-out}. Recall that $B'$ contains exactly one vertex
...
%
%  \item Refer to \figref{toughie}.  For an edge $uw\in E(G)$, let
...
%   cycle, $a\not\in B$ and $b\in B$.
%%   The only difference in the proof
...
%%   in $B$.  
%   This stronger version of \lemref{even-biconnected} is used
...
\begin{proof}
  ...
  Otherwise, we select a 2-connected component, $C$, that
...
In this section, we reuse the ideas from Bar\'at and Czap
...
\thmref{outerplane} provides a \emph{facial} nonrepetitive 11-colouring
...
set in a biconnected outerplane graph (\lemref{even-biconnected}) and
(2)~colouring cactus graphs with no odd cycles (\lemref{cactus}). The
proof of \lemref{cactus} is easily made into a linear-time algorithm. The
proof of \lemref{even-biconnected} can be implemented by a recursive
ear-cutting algorithm that implements \lemref{biconnected} followed by
a traversal of the dual tree in order to find the face $F_{uw}$ used in
the proof of \lemref{even-biconnected}.
...
\lemref{hitting_plus_four} uses a nonrepetitive 4-colouring of trees, but a
