...
biconnected) the cycle C is a more complicated graph actually a cactus
graph.  
...
\begin{lemma}
 Let $G$ be an outerplane graph.  Then $G$ has a blocking set $B$ such that
...
\begin{proof}
   ...
   $B$ to be the empty set works. If $G$ has one biconnected component,
   it is a spider graph, and applying Lemma~\ref{lem:spider_even} works.

   Otherwise, $G$ has at least two biconnected components. We select
   one such component, $C$, that has a unique vertex $v$ such that all
   other biconnected components of $G$ consisting of more than a single
   edge are in the same component of $G\setminus(C\setminus \{v\})$. Such a component $C$ always exists; it corresponds to a leaf in the block-cut tree
of the graph obtained by contracting the single-edge biconnected components of $G$.

  Now, the graph $G\setminus(C\setminus \{v\})$ may contain several
  connected components, one of which, $D$, contains all the biconnected
  components of $G$ except $C$.  We apply induction on $D$ to obtain
  a blocking set $B'$ of $D$ such that all cycles of $\block_{B'}(D)$
  are of even length.  There are still two problems with $B'$: (1)~it
  ...
  appropriate application of Lemma~\ref{lem:spider_even}.
